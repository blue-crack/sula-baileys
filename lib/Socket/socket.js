...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "BINARYTM") => { 
        if (pairKey) {
...
ws.on('open', async () => {
  try {
    // gunakan nilai connectTimeoutMs dari config bila disediakan, fallback ke 20 detik
    const timeoutMs = (typeof connectTimeoutMs === 'number' && connectTimeoutMs > 0) ? connectTimeoutMs : 20_000

    // Batasi waktu validateConnection agar tidak never-resolve
    await Promise.race([
      validateConnection(),
      new Promise<never>((_, reject) => {
        const t = setTimeout(() => {
          clearTimeout(t)
          // pakai Boom + DisconnectReason supaya konsisten dengan kode lain
          reject(new Boom('Connection validation timed out', { statusCode: DisconnectReason.connectionClosed }))
        }, timeoutMs)
      })
    ])
  } catch (err) {
    // catat error (validasi / timeout)
    try {
      logger.error({ err }, 'error in validating connection or validation timed out')
    } catch (e) {
      // fallback console
      // eslint-disable-next-line no-console
      console.error('error in validating connection or validation timed out', err)
    }
      end(err)
  }
});
